...
#minimized.gro dfpc_JANUS_2.xtc
# input
nprocs = 1
top  = 'min_bilayer_NP.gro'
traj = 'traj_comp.xtc'
...
def get_NP_coordinates(frame):
    u = MDAnalysis.Universe(top,traj)
    u.trajectory[frame]
    NP = 'NP'
    NP_COM = u.select_atoms('resname %s'%NP).center_of_geometry()
    return NP_COM
...
for i in range(len(NP_results)):
    NP_coordinates.append(NP_results[i])
    
for i in range(len(results)):
    atom_psi6s.append(results[i][0])
    atom_angles.append(results[i][1])
    coordinates.append(results[i][2])
...
if side == 'up':
    ...
    np.save('coordinate_upper_tail.npy', coordinates)
    np.save('NP_coordinates.npy', NP_coordinates)

elif side == 'down':
    np.save('psi6s_lower_tail.npy', atom_psi6s)
    np.save('angles_lower_tail.npy', atom_angles)
    np.save('coordinate_lower_tail.npy', coordinates)
    np.save('NP_coordinates.npy', NP_coordinates)
