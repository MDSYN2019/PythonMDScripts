...
for index in tqdm(range(1,len(indices))):
	data = []
...
for ind1, data in tqdm(enumerate(outputData)):
	replacementdata = []
	for line in data:
...
tempdat = outputData[0]
types = list(set(tempdat['Residue']))
NPdat = tempdat[tempdat['Residue'] == 'NP']
NPindices = list(set(NPdat['ResidueID']))
