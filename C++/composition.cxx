...
public:
  ...
  void storeFile() {    
	...
	if (l < 5) {	
	  ...
	  a[SSno].push_back(index); // Push back indices 
	  b[SSno].push_back(atomtype); // Push back atomtypes
	  xco[SSno].push_back(x*boxlength[0]); // Push back boxlengths - x coordinates
	  yco[SSno].push_back(y*boxlength[1]); // Push back boxlengths - y coordinates
	  zco[SSno].push_back(z*boxlength[2]); // Push back boxlengths - z coordinates
	  n++;
	  l++;
  ...
  double ChainOrderAnalysis(double* H7_x, double* H7_y, double* H7_z, double* H6_1_x, double* H6_1_y, double* H6_1_z,  double* H6_2_x, double* H6_2_y, double* H6_2_z, double* T3_1_x, double* T3_1_y, double* T3_1_z,  double* T3_2_x, double* T3_2_y, double* T3_2_z, double* T3_3_x, double* T3_3_y, double* T3_3_z, double* T4_x, double* T4_y, double* T4_z) {
    ...
    void clusterAnalysis(vector<double>* C12E2xCOM, vector<double>* C12E2yCOM, vector<double>* C12E2zCOM, std::vector<C12E2_skeleton>* values, double* distance, int* PolymerCounter) { 
...
private:    
  ...
  const int indexCG = 7;
  const int numberofSS = 1000; /*The number of screenshots in the dump file*/
